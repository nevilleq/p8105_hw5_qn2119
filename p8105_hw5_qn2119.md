...
With regards to these raw data pulled from the Github repository associated with this Washington Post article, there existed 52179 homicide case report observations in 12 variables describing the date, city, state, id, geospatial location, and result of the case, as well as the name, race, and gender of the victim. Initially, there actually existed 51 unique city, state locations; although this was identifed as an incorrectly imputted "Tulsa, AL" observation, which is not a city in Alabama. After initial tidying and cleaning, this incorrect point was identified and altered to describe "Tulsa, OK" by geospatial coordinates, a variable for "City, State" was created, an indicator for whether the case was unsolved was added, and we summarized the number of homicides unsolved and the number of total homicides, grouped by state. This resulted in a final 50 city/state observations in 3 variables, city/state, number of homicides unsolved, and total number of homicides.
...
<img src="p8105_hw5_qn2119_files/figure-markdown_github/unnamed-chunk-3-1.png" width="90%" style="display: block; margin: auto;" />
