...
read.p1.data <- function(data.csv.path){
  ...
  mutate(
    ...
    group = data.csv.path %>% rep(., length = length(week)) %>% stringr::str_extract(., "[ce][ox][np]")
  ) %>%
...
p1.spaghetti <- p1.df %>%
  ...
  geom_line(aes(linetype = group), alpha = 0.8, size = 1) +
  geom_smooth(aes(linetype = group), alpha = 1, method = "lm", se = FALSE, colour = "black", size = 1.5) +
     labs(
    x = "Week",
    y = "Observed Value",
